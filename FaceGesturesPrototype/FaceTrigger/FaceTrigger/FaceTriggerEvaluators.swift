...
class FrownEvaluator: FaceTriggerEvaluatorProtocol {
    ...
    func evaluate(_ blendShapes: [ARFaceAnchor.BlendShapeLocation : NSNumber], forDelegate delegate: FaceTriggerDelegate) {
        ...
        if let mouthFrownLeft = blendShapes[.mouthFrownLeft], let mouthFrownRight = blendShapes[.mouthFrownRight] {
            
            let newValue = ((mouthFrownLeft.floatValue + mouthFrownRight.floatValue) / 2.0) >= threshold
            if newValue != oldValue {
                delegate.onFrownDidChange?(frowning: newValue)
                if newValue {
                    delegate.onFrown?()
                }
            }
